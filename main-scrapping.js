#!/usr/bin/env node
/**
 * main-scrapping.js (ESM, production-ready)
 *
 * - Fully ESM entrypoint.
 * - Preserves original logic and features (scrapeProfile, scrapeRepos, scrapeUser,
 *   optional translator integration, CLI).
 * - Improved error messages, runtime dependency detection, and maintainability.
 * - Use this file for ESM consumers (import).
 *
 * Note:
 * - For CommonJS (`require(...)`) consumers, use the companion file `main-scrapping.cjs`
 *   which re-implements the same API in CommonJS shape. package.json provides
 *   conditional exports so both import/require work.
 */
...
// Runtime packages required for operation (names only)
...
async function loadRuntimes() {
  ...
  if (missing.length) {
    ...
    // Detect CLI execution (rough but practical check in ESM)
...
async function loadTranslatorModule() {
  ...
  try {
    ...
    try {
      ...
      // wrapper missing or failed; fallthrough to package import
    }

    // fallback: import optional package directly (may be ESM or CJS)
